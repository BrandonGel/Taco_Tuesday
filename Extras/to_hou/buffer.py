...
class ReplayBuffer(object):
    ...
    """
    def __init__(self, max_steps, num_agents, obs_ac_filter_loc_dims, is_cuda):
        """
        Inputs:
        ...
        """
        if len(obs_ac_filter_loc_dims) == 2:
            self.save_filter_loc = False
            obs_dims = obs_ac_filter_loc_dims[0]
            ac_dims = obs_ac_filter_loc_dims[1]
        elif len(obs_ac_filter_loc_dims) == 4:
            self.save_filter_loc = True
            obs_dims = obs_ac_filter_loc_dims[0]
            ac_dims = obs_ac_filter_loc_dims[1]
            filter_dims = obs_ac_filter_loc_dims[2]
            loc_dims = obs_ac_filter_loc_dims[3]
        else:
            pass
    ...
    def push_filter(self, observations, actions, rewards, next_observations, dones, filter_save, prisoner_save):
        nentries = 1  # handle multiple parallel environments (num of environments)
        if self.curr_i + nentries > self.max_steps:
            rollover = self.max_steps - self.curr_i # num of indices to roll over
            for agent_i in range(self.num_agents): # np.roll: rolling end to start by the unit of rollover
        ...
        for agent_i in range(self.num_agents): # for each agent
            ...
            self.next_obs_buffs[agent_i][self.curr_i:self.curr_i + nentries] = next_observations[agent_i]
            self.done_buffs[agent_i][self.curr_i:self.curr_i + nentries] = dones
            self.filtering_buffs[agent_i][self.curr_i:self.curr_i + nentries] = filter_save[agent_i]
            self.loc_buffs[agent_i][self.curr_i:self.curr_i + nentries] = prisoner_save[agent_i]

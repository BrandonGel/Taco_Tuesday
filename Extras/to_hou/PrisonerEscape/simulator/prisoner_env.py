...
from simulator.observation_spaces import transform_blue_detection_of_fugitive, transform_two_blue_detection_of_fugitive, create_partial_observation_space_blue_team
...
class RewardScheme:
    time: float = -5e-4
    detected = 2.0
    known_detected: float = 1.0
    known_undetected: float = -0.5
...
class PrisonerBothEnv(gym.Env):
    ...
    *Detection is encoded by a three tuple [b, x, y] where b in binary. If b=1 (detected), [x, y] will have the detected location in world coordinates. If b=0 (not detected), [x, y] will be [-1, -1].
    ...
    def __init__(self,
        ...
        self.unknown_hideout_pic_cv = cv2.imread("simulator/assets/star_blue.png", cv2.IMREAD_UNCHANGED)
    ...
    def step_both(self, red_action: np.ndarray, blue_action: np.ndarray):
        ...
        self._fugitive_observation = self._construct_fugitive_observation(red_action, fugitive_detection_of_parties, self._terrain_embedding)
        ...
        parties_detection_of_fugitive_one_hot = transform_two_blue_detection_of_fugitive(parties_detection_of_fugitive, self.prisoner_detected_loc_history2)
        ...
        active_detected, detect_num = self.is_fugitive_active_detected(active_parties_detection_of_fugitive)
        total_reward = self.get_reward(active_detected, detect_num)
    ...
    def step_partial_blue_obs(self, red_action: np.ndarray, blue_action: np.ndarray):
        ...
        parties_detection_of_fugitive = self._determine_blue_detection_of_red(speed)
    ...
    def get_reward(self, active_detected, detect_num):
        reward = 0
        ...
        for i, search_party in enumerate(self.search_parties_list):
            ...
            reward = reward - 0.1 * dist # original: 0.01
        for i, helicopter in enumerate(self.helicopters_list):
            dist = np.linalg.norm(np.array(helicopter.location)/2428.0 - np.array(self.prisoner.location)/2428.0)
            reward = reward - 0.5 * dist # original: 0.05
             
    # """ This setting is for on-policy training """         
    # def get_reward(self, active_detected, detect_num):
    #     reward = 0
    #     # TODO recode this so combinations of scenarios are possible per timestep
    #     # if self.timesteps == self.max_timesteps:
    ...
    #     for i, search_party in enumerate(self.search_parties_list):
    #         dist = np.linalg.norm(np.array(search_party.location)/2428.0 - np.array(self.prisoner.location)/2428.0)
    #         reward = reward - 0.01 * dist # original: 0.01
    #     for i, helicopter in enumerate(self.helicopters_list):
    #         dist = np.linalg.norm(np.array(helicopter.location)/2428.0 - np.array(self.prisoner.location)/2428.0)
    #         reward = reward - 0.05 * dist # original: 0.05
    ...
    def reset(self):
        ...
        self._fugitive_observation = self._construct_fugitive_observation([0.0, 0.0], fugitive_detection_of_parties, self._terrain_embedding)
        ...
        # parties_detection_of_fugitive_one_hot = transform_blue_detection_of_fugitive(parties_detection_of_fugitive, self.prisoner_detected_loc_history)
        """Two prisoner detected history to observation"""
        parties_detection_of_fugitive = transform_two_blue_detection_of_fugitive(parties_detection_of_fugitive, self.prisoner_detected_loc_history2)
        self._blue_observation = self._construct_blue_observation(parties_detection_of_fugitive, self._terrain_embedding, self.include_start_location_blue_obs)
        self._partial_blue_observation = self.__construct_partial_blue_observation(self.prisoner_detected_loc_history2)
        ...
        assert self._prediction_observation.shape == self.prediction_observation_space.shape, "Wrong observation shape %s, %s" % (self._fugitive_observation.shape, self.fugitive_observation_space.shape)

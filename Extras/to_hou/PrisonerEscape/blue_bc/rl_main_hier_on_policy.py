...
from blue_bc.policy import MLPNetwork, HighLevelPolicy, HierRandomPolicy
...
from buffer import Buffer, RolloutBuffer, RolloutBufferHier
...
def main_scratch(args):
    ...
    on_policy_optimizer = torch.optim.Adam(blue_policy.parameters(), lr=args.lr)
    """Initialize HierTrainer"""
    # hier_trainer = HierTrainer(blue_policy, input_dim=hier_act_dim+env.blue_observation_space.shape[0], out_dim=1, hidden_dim=128, update_target_period=5, lr=args.lr, device=device)
    for ep in range(args.episode_num):
        blue_hier_policy.reset()
        ...
        while not done:
            ...
            # action = blue_policy(torch.Tensor(next_blue_partial_observation).cuda())
            """Full Blue Obs"""
            action, new_detection, hier_action, log_pi = blue_hier_policy.predict_full_observation_random(blue_observation)
            next_blue_observation, next_blue_partial_observation, reward, done, _ = env.step(action)
            # print("hier_action.max() = ", hier_action.max())
            if new_detection is not None:
                loc_in_buffer = hier_buffer.append(blue_observation, hier_action.detach().cpu().numpy(), reward, done, log_pi, next_blue_observation)
                locs_in_buffer.append(loc_in_buffer)
            else:
                hier_buffer.append_rew(reward)
        ...
        if locs_in_buffer != []:
            loss = 0
            for start_idx in locs_in_buffer:
                accumulated_reward = hier_buffer.accumulate_reward(start_idx, args.gamma).detach()
                log_pi = hier_buffer.get_property(start_idx, "log_pis")
                loss =  loss + (- (log_pi * (accumulated_reward).to(device)).sum())
                # print("state = ", hier_buffer.get_property(start_idx, "states"))
                print("accumulated_reward = ", accumulated_reward)
                print("log_pi = ", log_pi)
            loss = loss / (len(locs_in_buffer))
            on_policy_optimizer.zero_grad()
            print("loss = ", loss)
            loss.backward()
            torch.nn.utils.clip_grad_norm(blue_policy.parameters(), 1.0)
...
if __name__ == '__main__':
    ...
    p.add_argument('--subpolicy_buffer_size', type=int, default=4500) # original 50000
    p.add_argument('--episode_num', type=int, default=50000) # original 5000
    ...
    p.add_argument('--lr', type=float, default=0.001) # original 0.01
    p.add_argument('--gamma', type=float, default=0.99) # original 0.01

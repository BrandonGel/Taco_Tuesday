...
class GAIL(PPO):
    ...
    def update(self, writer):
        ...
        # Calculate rewards.
        rewards = self.disc.calculate_reward(states, actions) # higher reward means looking more like expert
    ...
    def update_disc(self, states, actions, states_exp, actions_exp, writer):
        ...
        # print("states_exp = ", states_exp)
        # print("actions_exp = ", actions_exp)

        # Discriminator is to maximize E_{\pi} [log(1 - D)] + E_{exp} [log(D)].
        # See the logsigmoid function here: https://pytorch.org/docs/stable/generated/torch.nn.LogSigmoid.html (English)
        loss_pi = -F.logsigmoid(-logits_pi).mean() # loss_pi goes higher when logits_pi goes higher
        loss_exp = -F.logsigmoid(logits_exp).mean() # loss_exp goes lower when logits_exp goes higher
        loss_disc = loss_pi + loss_exp # Total discriminator loss

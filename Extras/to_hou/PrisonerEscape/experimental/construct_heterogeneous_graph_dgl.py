...
def convert_batched_dgl_graph_to_pyg(bg):
    ...
    data=HeteroData()
    edge_list = [('agent', 'to', 'agent_summ'), ('hideout', 'to', 'hideout_summ'), ('hideout_summ', 'to', 'state_summ'), ('agent_summ', 'to', 'state_summ'), ('timestep', 'to', 'state_summ')]
...
for node_type, I in i_dict.items():
    # val = I + conv_i(h_dict, edge_index_dict)
    # print(type(I))
    conv_val = conv_i(h_dict, edge_index_dict)[node_type]
    # print(type(conv_val))
    print(conv_val)
...
layer = HeteroGCLSTM(in_channels_dict = in_channels_dict, out_channels = 16, metadata = metadata)

layer(pyg.x_dict, pyg.edge_index_dict)
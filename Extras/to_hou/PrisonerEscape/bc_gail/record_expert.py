...
def generate_expert_traj(model, save_path=None, env=None, n_timesteps=0,
    ...
    if n_timesteps > 0 and isinstance(model, BaseRLModel):
        model.learn(n_timesteps) # Will not come here if use heuristic
    ...
    while ep_idx < n_episodes:
        ...
        # Use only first env
        if is_vec_env: # It is false and never comes here
    ...
    if isinstance(env.observation_space, spaces.Box) and not record_images:
        observations = np.concatenate(observations).reshape((-1,) + env.observation_space.shape) # it comes here
